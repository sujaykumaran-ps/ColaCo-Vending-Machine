...
handleCostUpdate = () => {
    ...
    if (costNumber > 99) {
        this.setState({ error: '*Cost Too High!!' });
        return;
    }
    const updatedVirtualSodas = virtualSodas.map(soda => {
    if (soda.name === selectedSoda.name) {
        ...
        .then(response => {
            if (!response.ok) {
            throw new Error('Failed to update soda cost');
            }
            return response.json();
        })
        .then(updatedSoda => {
            // update the virtualSodas array with the updated soda cost
            return {
            ...soda,
    ...
    // update the state with the updated virtualSodas array, quantity and cost
    this.setState({
        quantity: 0,
        cost: 0,
        virtualSodas: updatedVirtualSodas,
            ...
            <div className="vending-machine">
                ...
                <div className={selectedSoda ? "update-div" : "hidden"}>
                    ...
                    <div className={`cost-div ${selectedSoda?.name.split(' ').join('')}-update`}>
                        ...
                        <input type="text" id="cost" name="cost" defaultValue={defaultCost} onChange={this.handleCostChange}></input> 
                        <button type="button" onClick={this.handleCostUpdate}>Update Cost</button>
                        {this.state.error && (
                        <p className="error-message-restock">{this.state.error}</p>
                        )}
                    </div>
                </div>
                <div className={selectedSoda ? "hidden" : "purchase-button"}>
